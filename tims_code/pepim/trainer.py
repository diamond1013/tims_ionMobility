...
def data_to_tensors(data, im_min=0.5, im_max=1.5):
    peptides = data[["sequence"]].values.flatten()
    rt = data[["ionMobility"]].values    
    x = models.peptide_to_tensor(peptides)
    y = models.normalize(rt, min=im_min, max=im_max)
    return x, y
...
class PeptideIonMobilityTrainer:

    def __init__(self, model_path=None, model=None, im_min=0.5, im_max=1.5, save_path="bestmodel.hdf5", save_best_only=True, log_path='training.log'):
    ...
    def train(self, data, epochs=100, patience=15, validate_percent=.33, seed=0):
        x, y = data_to_tensors(data, im_min=self.im_min, im_max=self.im_max)
